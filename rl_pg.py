...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def forward(self, batch: TrainBatch, action_scores: torch.Tensor, **kw):
        ...
        p_weight = self._policy_weight(batch)
        loss_p = self._policy_loss(batch, action_scores, p_weight)
        # ========================
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        # ====== YOUR CODE: ======
        n = policy_weight.shape[0]
        actions_taken = batch.actions
        log_proba = nn.functional.log_softmax(action_scores, dim=1)
        log_predicted = log_proba.gather(dim=1, index=actions_taken)
        loss_p = policy_weight * log_predicted
        loss_p = loss_p.sum() / (-n)
...
class BaselinePolicyGradientLoss(VanillaPolicyGradientLoss):
    def forward(self, batch: TrainBatch, action_scores: torch.Tensor, **kw):
        ...
        p_weight, baseline = self._policy_weight(batch)
        loss_p = self._policy_loss(batch, action_scores, p_weight-baseline)
        # ========================
    ...
    def _policy_weight(self, batch: TrainBatch):
        ...
        # ====== YOUR CODE: ======
        n = batch.q_vals.shape[0]
        policy_weight = batch.q_vals
        baseline = torch.sum(batch.q_vals, dim=0) / n
        # ========================
...
class ActionEntropyLoss(nn.Module):
    ...
    def calc_max_entropy(n_actions):
        ...
        # ====== YOUR CODE: ======
        max_entropy = torch.log(torch.tensor(float(n_actions)))
        # ========================
        return max_entropy
    ...
    def forward(self, batch: TrainBatch, action_scores, **kw):
        ...
        # ====== YOUR CODE: ======
        log_proba = nn.functional.log_softmax(action_scores, dim=1)
        proba = nn.functional.softmax(action_scores, dim=1)
        max_entropy = self.calc_max_entropy(action_scores.shape[1])

        loss = torch.sum(log_proba * proba, dim=1)
        n = loss.shape[0]
        loss_e = loss.sum() / (n * max_entropy)
        # ========================
