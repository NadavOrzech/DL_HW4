...
class Episode(object):
    ...
    def calc_qvals(self, gamma: float) -> List[float]:
        ...
        qvals = [0]*n
        last_qval = 0
        for idx, exp in enumerate(reversed(self.experiences)):
            qval = last_qval*gamma + exp.reward
            qvals[n-idx-1] = qval
            last_qval = qval
        # ========================
        return qvals
...
class TrainBatch(object):
    ...
    def from_episodes(cls, episodes: Iterable[Episode], gamma=0.999):
        ...
        states, actions, qvals, total_reward = [], [], [], []
        for eps in episodes:
            states += [exp.state for exp in eps.experiences]
            actions += [exp.action for exp in eps.experiences]
            qvals += eps.calc_qvals(gamma)
            total_reward.append(eps.total_reward)

        states = torch.FloatTensor(torch.stack(states,dim=0))
        actions = torch.LongTensor(actions).reshape(-1,1)
        total_reward = torch.FloatTensor(total_reward).reshape(-1,1)
        qvals = torch.FloatTensor(qvals).reshape(-1,1)
